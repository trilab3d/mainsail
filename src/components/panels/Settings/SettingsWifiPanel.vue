...
<template>
	<v-card outlined :disabled="this.uiFrozen">
		...
		<v-card-text>
			<v-autocomplete v-model="wifiCountry" :items="items" item-title="text" item-value="value" :label="$t('panel.settingsWifi.country')"
				persistent-hint single-line></v-autocomplete>

			<v-select v-model="wifiMode" :items="wifi_mode_items" item-title="text" item-value="value" :label="$t('panel.settingsWifi.mode')"
			...
			<fieldset class="pr-3 pl-3 pb-3">
				...
				<v-combobox v-model="clientSSID" :items="clientSSIDArray" :placeholder="$t('panel.settingsWifi.ssid')" :menu-props="{
				...
				@change="autoselectMenu = false" @focusout="autoselectMenu = false">
				</v-combobox>
				<v-text-field :rules="passwordRules" v-model="client_pw" type="password"
					:label="$t('panel.settingsAccessPassword.password')" hide-details></v-text-field>
			</fieldset>
			<fieldset class="pr-3 pl-3 pb-3 mt-3">
				<legend>{{ $t('panel.settingsWifi.accessPoint') }}</legend>
				<v-text-field v-model="wifi_ap_ssid" item-title="text" item-value="value" :label="$t('panel.settingsWifi.ssid')"></v-text-field>
				<v-text-field :rules="passwordRules" v-model="pw" type="password"
					:label="$t('panel.settingsAccessPassword.password')" hide-details></v-text-field>
				<v-text-field :rules="confirmPasswordRules" v-model="confirmPassword"
					type="password" :label="$t('panel.settingsAccessPassword.confirmPassword')" hide-details></v-text-field>
			</fieldset>
		</v-card-text>
</v-card>
</template>
...
export default {
	computed: {
		...
		clientSSID: {
			get() {
				return this.settings.network_data.wifi_sta_ssid;
			},
			set(value) {
				this.assignSetting({ network_data: { "wifi_sta_ssid": value }});
			}
		},
		clientSSIDList: {
			get() {
		...
		wifiMode: {
			get() {
				return this.settings.network_data.wifi_mode;
			},
			set(value) {
				this.assignSetting({ network_data: {"wifi_mode": value }});
			}
		},
		wifiCountry: {
			get() {
				return this.settings.network_data.wifi_country;
			},
			set(value) {
				this.assignSetting({ network_data: {"wifi_country": value }});
			}
		},
		wifi_sta_ssid: {
			get() {
		...
		wifi_ap_ssid: {
			get() {
				return this.settings.network_data.wifi_ap_ssid;
			},
			set(value) {
				this.assignSetting({ network_data: { "wifi_ap_ssid": value }});
			}
		},
		client_pw: {
			get() {
				return this.settings.network_data.wifi_sta_pass;
			},
			set(value) {
				this.assignSetting({network_data: { "wifi_sta_pass": value }});
			}
		},
		pw: {
			get() {
				return this.settings.network_data.wifi_ap_pass;
			},
			set(value) {
				this.pass = value;
				if (this.confPass == value) {
					this.assignSetting({network_data: { "wifi_ap_pass": value }});
				}
			}
		},
		confirmPassword: {
	...
	methods: {
		...
		getWifiList() {
			axios.get(this.trilabPrefix + "/network/wifi/list")
				.then(response => {
