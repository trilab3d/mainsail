<template>
	<v-card outlined>
		...
		<v-card-text>
			<v-switch :label="$t('App.Trilab.Panels.TrilabAccessPassword.enablePwdControl')" hide-details
				v-model="enabledPasswordControl"></v-switch>

			<v-row :dense="$vuetify.breakpoint.mobile">
				<v-col cols="12" sm="12">
					<v-text-field type="text" v-model="username" :readonly="!enabledPasswordControl" :label="$t('App.Trilab.Panels.TrilabAccessPassword.username')"
						hide-details></v-text-field>
					<v-text-field :readonly="!enabledPasswordControl" :rules="passwordRules" v-model="password"
						type="password" :label="$t('App.Trilab.Panels.TrilabAccessPassword.password')" hide-details></v-text-field>
					<v-text-field :readonly="!enabledPasswordControl || password.trim() == ''" :rules="confirmPasswordRules"
						...
						:label="$t('App.Trilab.Panels.TrilabAccessPassword.confirmPassword')" hide-details></v-text-field>
					<v-divider class="my-2"></v-divider>
					<v-btn block primary @click="showWarningDialog = true" :disabled="!passwordsOk">{{ $t('App.Trilab.SettingsAccessPasswordPanel.SaveBtn') }}</v-btn>
				</v-col>
			</v-row>
			<v-dialog v-model="showWarningDialog" max-width="400">
				<v-card>
					<v-card-title class="headline">{{ $t('App.Trilab.Panels.TrilabAccessPassword.Warning') }}</v-card-title>
					<v-card-text>
						<p>{{ $t('App.Trilab.Panels.TrilabAccessPassword.DialogMessage') }} </p>
					</v-card-text>
					<v-card-actions>
						<v-spacer></v-spacer>
						<v-btn color="red darken-1" text @click="showWarningDialog = false">{{ $t('App.Trilab.Panels.TrilabAccessPassword.DialogNoButton') }}</v-btn>
						<v-btn color="green darken-1" text @click="showWarningDialog = false; saveSettings()">{{ $t('App.Trilab.Panels.TrilabAccessPassword.DialogYesButton') }}</v-btn>
					</v-card-actions>
				</v-card>
			</v-dialog>
