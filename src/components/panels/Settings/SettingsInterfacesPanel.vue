...
<template>
    <v-card
        ...
        <v-container
            v-if="activeView == 'interface_list'"
            class="">
            ...
            <v-list>
                <template v-for="(i, index) in interfaces">
                    <v-subheader
                        v-if="i.header"
                        :key="'interfaceheader' + index"
                        v-text="i.header"></v-subheader>
                    <v-list-item
                        :key="i.GENERAL.DEVICE"
                        :index="index"
                        :item="i">
                        <v-list-item-avatar>
                            <v-icon
                                v-if="i.GENERAL.TYPE == 'wifi'"
                                size="x-large">
                                {{ mdiWifi }}
                            </v-icon>
                            <v-icon
                        ...
                        <v-list-item-content>
                            ...
                            <v-list-item-subtitle>
                                <small v-if="i.GENERAL.TYPE == 'wifi'">
                                    SSID: {{ i.GENERAL.CONNECTION }}
                                    <br />
                                </small>
                                <div v-if="'IP4' in i">
                                    <small v-if="'ADDRESS' in i.IP4">
                                        {{ $t('App.Trilab.Panels.TrilabSettingsInterfaces.IP4addresses') }}:
                                        {{ i.IP4.ADDRESS.join(',') }}
                                        <br />
                                    </small>
                                </div>
                                <div v-if="'IP6' in i">
                                    <small v-if="'ADDRESS' in i.IP6">
                                        {{ $t('App.Trilab.Panels.TrilabSettingsInterfaces.IP6addresses') }}:
                                        {{ i.IP6.ADDRESS.join(',') }}
                                        <br />
                                    </small>
        ...
        <v-container v-if="activeView == 'connections_list'">
            ...
            <v-row class="d-flex mb-2">
                ...
                <v-col>
                    <v-btn
                        block
                        @click="listWifi()"
                        v-if="selectedInterface.GENERAL.TYPE == 'wifi'">
                        <v-icon class="mr-2">
                            {{ mdiWifiPlus }}
                        </v-icon>
            ...
            <h3>
                {{ $t('App.Trilab.Panels.TrilabSettingsInterfaces.ProfilesIn') }} {{ selectedInterface.GENERAL.DEVICE }}
            </h3>
                    ...
                    <v-list-item
                        ...
                        <v-list-item-content>
                            ...
                            <v-list-item-subtitle>
                                ...
                                <small v-if="i.ACTIVE == 'no'">
                                    {{ $t('App.Trilab.Panels.TrilabSettingsInterfaces.ProfileInactive') }}
                                    <br />
                                </small>
                            </v-list-item-subtitle>
                        </v-list-item-content>
                        ...
                        <v-list-item-action>
                            <v-row>
                                ...
                                <v-btn
                                    icon
                                    @click="removeProfile(selectedInterface, i, i.UUID)">
                        ...
                        <v-list-item-content>
                            ...
                            <v-list-item-subtitle>
                                ...
                                <v-container
                                    v-if="
                                        item.SECURITY != '--' &&
                                        item.show_password_container == true &&
                                        item.connecting == false
                                    ">
                                    <v-text-field
                                        v-model="item.user_selected_password"
                                        label="Password"
                                        @input="$forceUpdate()"></v-text-field>
                                    <v-btn
                                        @click="connectWifi(item)"
                                        :disabled="item.user_selected_password == ''"
                                        color="success">
                                        Connect
                                    </v-btn>
                                </v-container>
        ...
        <v-tabs
            v-if="activeView == 'connection_settings'"
            v-model="activeTab">
            <v-tab
                v-for="(tab, index) of visibleTabTitles"
                :key="index"
                :href="'#' + tab.name"
                style="width: 200px">
                <v-icon left></v-icon>
        ...
        <v-container
            ...
            <v-container v-if="activeTab == 'general'">
                <v-text-field
                    v-model="selectedConnectionCopy.details.connection.id"
                ...
                <v-switch
                    v-model="autoconnectValue"
                    label="Connect automatically"></v-switch>
                ...
                <v-text-field
                    v-if="selectedInterface.GENERAL.TYPE != 'wifi'"
                    v-model="selectedConnectionCopy.details.ethernet['cloned-mac-address']"
                    label="Cloned MAC address"
                    :rules="[rules.mac]"></v-text-field>
            </v-container>

            <v-container v-if="activeTab == 'ipv4'">
                <v-select
                    v-model="selectedConnectionCopy.details.ipv4.method"
                    item-value="value"
                    item-text="label"
                    :items="ipv4Methods"
                    label="IPv4 Method"></v-select>

                <v-text-field
                    v-if="
                        selectedConnectionCopy.details.ipv4.method == 'manual' ||
                        selectedConnectionCopy.details.ipv4.method == 'shared'
                    "
                    v-model="selectedConnectionCopy.details.ipv4.addresses"
                    label="IP"
                ...
                <v-text-field
                    v-if="
                        ...
                        selectedConnectionCopy.details.ipv4.method == 'shared'
                    "
                    v-model="selectedConnectionCopy.details.ipv4.gateway"
                    label="Gateway"
                    :rules="[rules.IPv4]"></v-text-field>

                <v-container
                    class="mx-0 px-0 px-0 py-0"
                    v-if="
                        selectedConnectionCopy.details.ipv4.method == 'manual' ||
                        selectedConnectionCopy.details.ipv4.method == 'shared' ||
                        selectedConnectionCopy.details.ipv4.method == 'auto'
                    ">
                    <v-container
                        v-for="(ip, index) in IPv4DNSList"
                        :key="index"
                        class="d-flex px-0 mx-0">
                        <v-text-field
                            v-model="IPv4DNSList[index]"
                            label="DNS"
                            :rules="[rules.IPv4]">
                            <template
                                v-if="index != 0"
            ...
            <v-container v-if="activeTab == 'ipv6'">
                <v-select
                    v-model="selectedConnectionCopy.details.ipv6.method"
                    item-value="value"
                    item-text="label"
                    :items="ipv4Methods"
                    label="IPv6 Method"></v-select>

                <v-text-field
                    v-if="
                        selectedConnectionCopy.details.ipv6.method == 'manual' ||
                        selectedConnectionCopy.details.ipv6.method == 'shared'
                    "
                    v-model="selectedConnectionCopy.details.ipv6.addresses"
                    label="IP"
                ...
                <v-text-field
                    v-if="
                        ...
                        selectedConnectionCopy.details.ipv6.method == 'shared'
                    "
                    v-model="selectedConnectionCopy.details.ipv6.gateway"
                    label="Gateway"
                    :rules="[rules.IPv6]"></v-text-field>
                ...
                <v-container
                    ...
                    v-if="
                        selectedConnectionCopy.details.ipv6.method == 'manual' ||
                        selectedConnectionCopy.details.ipv6.method == 'shared' ||
                        selectedConnectionCopy.details.ipv6.method == 'auto'
                    ">
                    <v-container
                        v-for="(ip, index) in IPv6DNSList"
                        :key="index">
                        <v-text-field
                            v-model="IPv6DNSList[index]"
                            label="DNS"
                            :rules="[rules.IPv6]"></v-text-field>
                        <!--- remove btn --->
                        <v-btn
                            icon
                            @click="IPv6DNSList.splice(index, 1)">
                            <v-icon>{{ mdiTrashCan }}</v-icon>
                        </v-btn>
                    </v-container>
                    <v-btn
                        icon
                        @click="IPv6DNSList.push('')">
                        <v-icon>{{ mdiPlus }}</v-icon>
                    </v-btn>
                </v-container>
            </v-container>
            ...
            <v-container v-if="activeTab == 'wireless'">
                ...
                <v-select
                    v-model="selectedConnectionCopy.details['wireless'].mode"
                    item-value="value"
                    item-text="label"
                    :items="wirelessModes"
                    label="Wireless mode"></v-select>

                <v-text-field
                    v-model="selectedConnectionCopy.details['wireless'].ssid"
                    label="SSID"></v-text-field>

                <!--- security methods --->
                <v-select
                    v-model="selectedConnectionCopy.details['wireless-security']['key-mgmt']"
                    item-value="value"
                    item-text="label"
                    :items="securityModes"
                    label="Security mode"></v-select>

                <!--- password --->
                <v-text-field
                    v-if="selectedConnectionCopy.details['wireless-security']['key-mgmt'] == 'wpa-psk'"
                    v-model="selectedConnectionCopy.details['wireless-security'].psk"
                    label="Password"></v-text-field>
                <span v-if="selectedConnectionCopy.details['wireless-security']['key-mgmt'] != 'wpa-psk'">
                    --- NOT YET IMPLEMENTED ---
                </span>
            </v-container>
...
export default class SettingsInterfacesPanel extends Mixins(BaseMixin, TrilabMixin) {
    ...
    selectConnection(connection: any) {
        ...
        this.IPv4DNSList = this.selectedConnectionCopy?.details.ipv4?.dns
            .split(',')
            .map((value: string) => value.trim())
        this.IPv6DNSList = this.selectedConnectionCopy?.details.ipv6?.dns
            .split(',')
            .map((value: string) => value.trim())
    ...
    onInterfacesChanged() {
        ...
        if (this.selectedInterface != null) {
            this.selectedInterface = this.$store.state.trilab.interfaces.find(
                (i: any) => i.GENERAL.DEVICE == this.selectedInterfaceCopy.GENERAL.DEVICE
            )
        }
    }
    ...
    saveChangedData() {
        ...
        this.selectedConnectionCopy.details.ipv4.dns =
            this.IPv4DNSList.filter((value: string) => value.trim() != '').join(',') ?? ''
        this.selectedConnectionCopy.details.ipv6.dns =
            this.IPv6DNSList.filter((value: string) => value.trim() != '').join(',') ?? ''

        const differences = this.findDataDifferences(
            this.selectedConnection.details,
            this.selectedConnectionCopy.details
        )
    ...
    async addProfile() {
        ...
        fetch(
            this.$store.getters['trilab/trilabPrefix'] +
                '/network-manager/add-connection/' +
                urlname +
                '/' +
                encodeURIComponent(conn_name),
            {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
