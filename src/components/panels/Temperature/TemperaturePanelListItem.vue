<template>
    <tr>
        ...
        <td class="current">
            ...
            <temperature-panel-list-item-additional-sensor v-if="additionalSensorName" :object-name="objectName"
                :additional-object-name="additionalSensorName" />
        </td>
        <td class="target">
            <temperature-input v-if="command !== null" :name="name" :target="target" :presets="presets"
                :min_temp="min_temp" :max_temp="max_temp" :command="command" :attribute-name="commandAttributeName" />
        </td>
        <temperature-panel-list-item-edit :bool-show="showEditDialog" :object-name="objectName" :name="name"
            :format-name="formatName" :additional-sensor-name="additionalSensorName" :icon="icon" :color="color"
...
export default class TemperaturePanelListItem extends Mixins(BaseMixin) {
    ...
    get icon() {
        if (this.objectName.startsWith('heater_chamber')) {
            return '$chamberIcon';
        }
