                ...
                <v-col
                        ...
                        <v-card outlined>
                            ...
                            <v-card-text>
                                <p>
                                    ...
                                    to start the basic calibration. Start it by pressing the button below and then wait
                                    for the printer to restart.
                                </p>
                                <v-btn
                                    :disabled="printerBusy"
                                    color="primary"
                                    :loading="loadings.includes('delta_calibrate')"
                                    block
                                    @click="sendBasicCalibrationCommand()">
                                    START CALIBRATION
                                </v-btn>
                            </v-card-text>
                        </v-card>
                    ...
                    <v-container v-if="step == 1">
                        <v-card outlined>
                            ...
                            <v-card-text>
                                Basic delta calibration generally calculates delta parameters well to ensure the nozzle
                                is at the correct distance from the bed. However, it does not attempt to calibrate the
                                accuracy of X and Y dimensions. For verifying dimensional accuracy, it is good to
                                perform an extended delta calibration.
                                <br />
                                This calibration procedure requires printing a test object and measuring parts of this
                                test object using digital calipers.
                                <br />
                                <strong>Make sure</strong>
                                that since the last basic delta calibration, there have been no significant changes in
                                the printer configuration or hardware (if are not sure, perform the basic delta
                                calibration again, just before printing the test object below).
                                <br />
                                <v-btn
                                    :disabled="printerBusy || loadings.includes('DeltaCalibrationWizardPrint')"
                                    block
                                    @click="sendPrintTestObject()">
                                    Print the test object
                                </v-btn>
                                <p class="mt-6 mb-2">Print the test object and wait until it completely cools down.</p>
                                <small>
                                    Ensure that the subsequent values are inputted Ensure that the subsequent values are
                                    inputted with the same printer settings used during the calibration object print (do
                                    not configuration between printing and measuring). If feasible, carry out the
                                    described measurements while the object is still attached to the print bed. However,
                                    do not worry if the part detaches from the bed - just strive to avoid bending the
                    ...
                    <v-container v-if="step == 2">
                        <v-card outlined>
                            ...
                            <v-card-text>
                                <p>
                                    If the object has been resized to a smaller or larger size, please provide the scale
                                    factor used during object slicing. If no resizing was done, leave the default value.
                                    <small>
                                        (A scale value of 2.0 would mean double the original size of the object, while a
                                        value of 0.5 would mean half the original size.)
                                    </small>
                                </p>

                                <p>Now enter the value (using decimals - Use a dot instead of a comma):</p>

                                <v-row>
                                    <v-text-field
                                        label="Měřítko velikosti"
                    ...
                    <v-container v-if="step == 3">
                        <v-card outlined>
                            ...
                            <v-card-text>
                                <p>
                                    Then proceed counterclockwise and measure the distances between the center column
                                    and the other columns (distance from the center to the column opposite the C label,
                                    distance from the center to the column labeled B, etc.).
                                </p>
                                <div
                                    style="text-align: center"
                                    class="my-3 mx-3">
                                    <img
                                        src="/img/delta-a-distance.jpg"
                                        alt="delta-a-distance"
                                ...
                                <small>
                                    (distance from the center to the column opposite the C label, distance from the
                                    center to the column labeled B, etc.)
                                </small>
                                :
                                <div
                                    style="text-align: center"
                                    class="my-3 mx-3">
                                    <img
                                        src="/img/delta_cal_e_step1.jpg"
                                        alt="step_1"
                                        style="width: 100%; max-width: 500px" />
                                    <p><small>Measurement - step 2</small></p>
                                </div>

                                <p>Now enter the measured values (using decimals - Use a dot instead of a comma):</p>
                        ...
                        <v-card outlined>
                            ...
                            <v-card-text>
                                ...
                                <div
                                    ...
                                    class="my-3 mx-3">
                                    <img
                                        src="/img/delta-outer-distance.jpg"
                                        alt="step_1"
                                        style="width: 100%; max-width: 500px" />
                                    <p><small>Measurement- - step 2</small></p>
                                </div>
                                ...
                                <v-row>
                                    <v-col
                                        v-for="(field, index) in far_fields"
                                        :key="index"
                                        cols="4"
                                        md="12"
                                        lg="4">
                    ...
                    <v-container v-if="step == 5">
                        <v-card outlined>
                            ...
                            <v-card-text>
                                <p>
                                    At this point, you can remove the object from the bed. The final measurements
                                    concern the columns themselves. Measure the size of the central column along arm A,
                                    then arm B, and finally arm C
                                </p>
                                <div
                                    style="text-align: center"
                                    class="my-3 mx-3">
                                    <img
                                        src="/img/delta-a-pillar.jpg"
                                        alt="step_3"
                                        style="width: 100%; max-width: 500px" />
                                ...
                                <v-row>
                                    <v-col
                                        v-for="(field, index) in pillarFields"
                                        :key="index"
                                        cols="4"
                                        md="6"
                                        lg="4">
                    ...
                    <v-container v-if="step == 6">
                        <v-card outlined>
                            ...
                            <v-card-text>
                                <p>
                                    The final dimensions relate to the outer pillars. Begin by measuring the distance of
                                    pillar and along the line from pillar A to the pillar opposite to pillar C.
                                </p>
                                <div
                                    style="text-align: center"
                                    class="my-3 mx-3">
                                ...
                                <p>
                                    Then proceed counterclockwise and measure the remaining outer pillars (the pillar
                                    diagonally from C along the line to B, pillar B along the line to the pillar
                                    diagonally from A, etc.).
                                </p>
                                ...
                                <v-row>
                                    <v-col
                                        v-for="(field, index) in outerPillarFields"
                                        :key="index"
                                        cols="2"
                                        md="12"
                                        lg="2">
                    ...
                    <v-container v-if="step == 7">
                        <v-card outlined>
                            ...
                            <v-card-text>
                                <p>
                                    Completion of this command may take several minutes. Upon completion, it will
                                    calculate updated delta parameters (delta radius, tower angles, endstop positions,
                                    and arm lengths). It will then save both the updated delta parameters and the
                                    distance measurement information. Future calibrations will also utilize this
                                    distance information. After pressing the button below, refrain from attempting to
                                    re-enter unprocessed distance measurements because this command alters the printer
                                    configuration, and unprocessed measurements will no longer be valid.
                                </p>
                                <p style="text-align: center; font-weight: bold">
                                    Upon pressing this button, this wizard and the saved values will reset, and any
                                    corrections will need to be made again from scratch.
                                </p>
                                <v-btn
                                    :disabled="printerBusy || sendCalibrateCommandEnabled == false"
                                    :loading="advancedCalibrationInProgress"
                                    color="red"
                                    class="mt-4"
                                    block
                                    @click="sendCalibrateCommand()">
                                    Start calibration
                                </v-btn>
                            </v-card-text>
                        </v-card>
...
export default class TrilabDeltaCalibrationWizard extends Mixins(BaseMixin, ControlMixin, TrilabMixin) {
    ...
    startOver() {
        if (
            confirm(
                'Opravdu chcete začít znovu? Smažou se všechna uložená data v databázi a wizard se vrátí na začátek'
            )
        ) {
    ...
    dbUrl(key: string = '', value: string = '') {
        var toReturn =
            this.$store.getters['socket/getUrl'] + '/server/database/item?namespace=trilabdeltacalibrationwizard'
        if (key != '') {
    ...
    async sendCalibrateCommand() {
        ...
        let center_dists = `DELTA_ANALYZE CENTER_DISTS=${this.getField('a_dist')},${this.getField(
            'far_c_dist'
        )},${this.getField('b_dist')},${this.getField('far_a_dist')},${this.getField('c_dist')},${this.getField(
            'far_c_dist'
        )}`
        let outer_dists = `DELTA_ANALYZE OUTER_DISTS=${this.getFarField('a_to_far_c')},${this.getFarField(
            'far_c_to_b'
        )},${this.getFarField('b_to_far_a')},${this.getFarField('far_a_to_c')},${this.getFarField(
            'c_to_far_b'
        )},${this.getFarField('far_b_to_a')}`
        let center_pillar_widths = `DELTA_ANALYZE CENTER_PILLAR_WIDTHS=${this.getPillarField(
            'center_pillar_a'
        )},${this.getPillarField('center_pillar_b')},${this.getPillarField('center_pillar_c')}`
        let outer_pillar_widths = `DELTA_ANALYZE OUTER_PILLAR_WIDTHS=${this.getOuterPillarField(
            'pillar_a'
        )},${this.getOuterPillarField('far_c')},${this.getOuterPillarField('pillar_b')},${this.getOuterPillarField(
            'far_a'
        )},${this.getOuterPillarField('pillar_c')},${this.getOuterPillarField('far_b')}`
    ...
    onEventsChange() {
        /// events
        if (this.advancedCalibrationInProgress == false) {
            return
        }
    ...
    skipStep() {
        if (
            confirm(
                'Opravdu chcete přeskočit tento krok? Provedení tohoto kroku nevadí, pokud již tato akce byla provedena'
            )
        ) {
            this.step = this.step + 1
        }
