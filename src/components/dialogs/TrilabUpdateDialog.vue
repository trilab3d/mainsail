        ...
        <v-card>
            ...
            <v-card-text>
                <div v-if="uploadError != null">
                    ...
                    <v-btn
                        v-if="!AdvancedFeatures || !showTechnicalDetails"
                        class="mt-2 mb-2 block"
                        color="red"
                        @click="showErrorDetails()">
                        {{ $t('App.Trilab.TrilabUpdateDialog.ShowErrorTechinicalDetails') }}
                    </v-btn>
                </div>
                <div v-if="uploadError == null">
                    <div v-if="alreadyUpdating == false">
                        ...
                        <div v-if="uploadProgress < 100">
                            <v-progress-linear
                                ...
                                :color="uploadFileProgressbarColor"
                                :height="20"
                                striped>
                    ...
                    <div v-if="alreadyUpdating == true">
                        <p>
                            {{ $t('App.Trilab.TrilabUpdateDialog.STATUS') }}
                            <span>{{ customStatus }}</span>
                        </p>
                        <v-progress-linear
                            :indeterminate="updateProgressStatus === 'IDLE'"
                        ...
                        <p
                            style="color: #f44336; text-align: center"
                            class="mt-4">
                            The printer is currently updating. Please do not turn off the device. The printer will
                            automatically restart after the update is completed.
                        </p>
                        <v-btn
                            color="danger"
                            v-if="showRestartBtn">
                            {{ $t('App.Trilab.TrilabUpdateDialog.RestartPrinter') }}
                        </v-btn>
                    </div>
                </div>
                <v-list v-if="AdvancedFeatures || showTechnicalDetails">
                    <v-list-item
                        ...
                        class="ulog">
                        <span
                            v-if="log.text.indexOf('ERROR') != -1"
                            style="color: #f44336">
                            {{ $t('App.Trilab.TrilabUpdateDialog.DANGER') }}
                        </span>
                        <span
                            v-else-if="log.text.indexOf('WARNING') != -1"
                            style="color: #ff9800">
                            {{ $t('App.Trilab.TrilabUpdateDialog.WARNING') }}
                        </span>
                        <span
                            v-else
                            style="color: #2196f3">
                            {{ $t('App.Trilab.TrilabUpdateDialog.INFO') }}
                        </span>
...
export default class TrilabUpdateDialog extends Mixins(BaseMixin, TrilabMixin) {
    ...
    onFileChanged(newFile: File | null) {
        if (newFile !== null && this.alreadyUpdating == false) {
            ...
            axios
                .post('http://' + this.$store.state.trilab.connectedHostname + '/swupdate/upload', formData, {
                    headers: {
    ...
    createSocket() {
        ...
        this.socket = new WebSocket(
            s +
                '//' +
                this.$store.state.trilab.connectedHostname +
                '/swupdate/' +
