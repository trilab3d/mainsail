        ...
        <v-card>
            ...
            <v-card-text>
                ...
                <div v-if="step == 0">
                    ...
                    <v-btn
                        :loading="probeCheckingInProgress"
                        block
                        color="primary"
                        @click="
                            sendAnalogProbeCommand()
                            probeCheckingInProgress = true
                        ">
                        Bed is empty, continue
                    </v-btn>
                    <p v-if="probeCheckingInProgress">Averaging the values, please wait...</p>
                </div>
                <div v-if="step == 1 && !waitingForChange">
                    <p>
                        Place 500 grams on place that is highlighted in following image. Then press "Check" and wait for
                        checking to complete
                    </p>
                    <img
                        src="/img/bedExampleA.jpg"
                        alt="A"
                    ...
                    <p class="mb-4 mt-4" style="text-align: center; font-weight: bold; font-size: 125%">
                        <span style="color: lime" v-if="probeARegistered">Registered</span>
                        <span color="red" v-if="!probeARegistered">{{ lastVal }}</span>
                    </p>
                ...
                <div v-if="step == 2 && !waitingForChange">
                    ...
                    <p class="mb-4 mt-4" style="text-align: center; font-weight: bold; font-size: 125%">
                        <span style="color: lime" v-if="probeBRegistered">Registered</span>
                        <span color="red" v-if="!probeBRegistered">{{ lastVal }}</span>
                    </p>
                ...
                <div v-if="step == 3 && !waitingForChange">
                    ...
                    <p class="mb-4 mt-4" style="text-align: center; font-weight: bold; font-size: 125%">
                        <span style="color: lime" v-if="endStopCRegistered">Registered</span>
                        <span color="red" v-if="!endStopCRegistered">{{ lastVal }}</span>
                    </p>
                </div>
                ...
                <div v-if="step == 4">
                    <p style="color: lime">
                        All points were successfully registered. Please remove the object from the bed and close the
                        wizard with the button below
                    </p>
                ...
                <div v-if="waitingForChange">
                    <p style="text-align: center; color: lime">
                        Now remove the heavy object from the desk {{ lastVal }}
                    </p>
                </div>

                <div>
                    <small v-if="valueWithoutWeightAvgd != 0">W/o weight: {{ valueWithoutWeightAvgd }}</small>
                    <br />
                </div>

                <div v-if="isCloseBtnVisible">
                    <v-divider class="mt-4 mb-4"></v-divider>
                    <v-btn color="primary" @click="fail()">
    ...
    onSocketResponsesChanged() {
        ...
        if (isNeededMessage != -1) {
            ...
            if (Math.abs(parseInt(parsedValue) - this.lastValBeforeChange) < 50 && this.lastValBeforeChange != null) {
                ...
                console.log(
                    'waiting for change reset because of difference of' +
                        this.lastValBeforeChange +
                        ' : vs now : ' +
                        parsedValue
                )
            }
            ...
            if (this.step == 0) {
                ...
                if (this.firstNoWeightArrayForAverage.length < 15) {
                    ...
                    this.valueWithoutWeightAvgd = Math.round(
                        this.firstNoWeightArrayForAverage.reduce((a: number, b: number) => a + b, 0) /
                            this.firstNoWeightArrayForAverage.length
                    )
                    ...
                    if (max - min > 400) {
                        this.$toast.error(
                            'Bed weight sensor value difference is more than 400 from min and max. The sensor is probably not working correctly.'
                        )
                        this.fail()
                        return
                    }
                ...
                if (this.checkRetries >= this.minCheckRetries) {
                    ...
                    if (
                        avgA < this.valueWithoutWeightAvgd + this.weightObjectToleranceMin ||
                        avgA > this.valueWithoutWeightAvgd + this.weightObjectToleranceMax
                    ) {
                        console.log('avgA: ' + avgA + ' valueWithoutWeightAvgd: ' + this.valueWithoutWeightAvgd)
                        this.$toast.error(
                            'Probe A is not working, the value is not in the expected range. Please check the sensor and try again.' +
                                'Current value: ' +
                                parsedValue +
                                ' First value with weight: ' +
                                this.firstValueWithWeight
                        )
                        this.fail()
                        return
                    }
                ...
                if (this.checkRetries >= this.minCheckRetries) {
                    ...
                    /// the avgA has to be min noWeightAvgd + 150 and less than +250 (from the variables)
                    if (
                        avgB < this.valueWithoutWeightAvgd + this.weightObjectToleranceMin ||
                        avgB > this.valueWithoutWeightAvgd + this.weightObjectToleranceMax
                    ) {
                        this.$toast.error(
                            'Probe B is not working, the value is not in the expected range. Please check the sensor and try again.' +
                                'Current value: ' +
                                parsedValue +
                                ' First value with weight: ' +
                                this.firstValueWithWeight
                ...
                if (this.checkRetries >= this.minCheckRetries) {
                    ...
                    if (
                        avgC < this.valueWithoutWeightAvgd + this.weightObjectToleranceMin ||
                        avgC > this.valueWithoutWeightAvgd + this.weightObjectToleranceMax
                    ) {
                        this.$toast.error(
                            'Probe C is not working, the value is not in the expected range. Please check the sensor and try again.' +
                                'Current value: ' +
                                parsedValue +
                                ' First value with weight: ' +
                                this.firstValueWithWeight
