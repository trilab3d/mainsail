    ...
    <v-dialog
        ...
        <v-card>
            ...
            <v-card-text>
                ...
                <div v-if="step == 0">
                    <p>
                        The filament is detected. On the start of this dialog, filament should not be detected. Please
                        remove the filament to continue
                    </p>
                </div>
                ...
                <img
                    v-if="step == 1 || step == 2"
                    src="/img/diagnostics/filamentTest.gif"
                    class="mt-3 mb-3"
...
export default class TrilabDiagnosticsFilamentSensorDialog extends Mixins(TrilabMixin) {
    ...
    onFilamentSensorStatusChanged(newValue: boolean, oldValue: boolean) {
        ...
        } else if (this.step == 1 && newValue == true) {
            this.step = 2
        }
    ...
    onIsDialogVisibleChanged(newValue: boolean, oldValue: boolean) {
        ...
        if (newValue == true && oldValue == false) {
            ...
            if (this.filamentSensorStatus == 0) {
                ///dialog should start from not detected (filament) step, so if it is detected on the start, give user a message to remove the filament before continuing
                this.step = 1
            }
        } else {
